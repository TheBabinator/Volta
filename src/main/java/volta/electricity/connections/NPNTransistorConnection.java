...
public abstract class NPNTransistorConnection extends BaseConnection {
    private final Terminal base;

    public NPNTransistorConnection(Terminal base) {
        this.base = base;
    }

    @Override
    public double getChargeFlow(Simulation simulation, Terminal positive, Terminal negative, double deltaTime) {
        double baseChargeFlow = Terminals.flowDiode(base, negative, getBaseVoltageDrop());
        base.setOnlyCharge(base.getCharge() - baseChargeFlow);
        negative.setOnlyCharge(negative.getCharge() + baseChargeFlow);
